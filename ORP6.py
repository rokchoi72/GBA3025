...
# All Weather Portfolio => ['SPY','TLT','IEF','GLD']
...
# 5) Charts
# (a) Price indexed to 100 (use first common valid date)
data_common = data.dropna(how='any')
(data_common / data_common.iloc[0] * 100).plot(figsize=(8,5), title='Price (Indexed to 100)')
plt.ylabel('Index Level')
plt.tight_layout()
plt.show()

# (b) Daily simple return distributions
rets = data.pct_change().dropna(how='any')
...
# guard for degenerate spread
if np.isclose(min_return, max_return):
    max_return = float(min_return) + 1e-6

trets = np.linspace(float(min_return), float(max_return), 50)
tvols = []
for tret in trets:
...
# ---------- Helpers ----------
def compute_drawdown_simple(equity: pd.Series):
...
def perf_from_equity(equity: pd.Series, rf: float):
    ...
    cagr    = (equity.iloc[-1] / equity.iloc[0])**(1/years) - 1
    dd      = compute_drawdown_simple(equity)
    max_dd  = float(dd.min())
...
if BM_MODE == 'SPY':
    if 'SPY' not in data.columns:
        spy_px = yf.download(
            bm_symbols_spy,
            start=rets.index[0],
            end=rets.index[-1] + pd.Timedelta(days=1),
            auto_adjust=False
        )['Adj Close']
...
dd_orp = compute_drawdown_simple(eq_orp_c)
dd_bm  = compute_drawdown_simple(eq_bm_c)
...
# Format: percentages for rate/vol/dd; ratio for Sharpe
summary_fmt = summary.copy()
for c in ["CAGR","Ann Ret","Ann Vol","Max DD"]:
    summary_fmt[c] = summary_fmt[c].map(lambda x: f"{x:.2%}")
summary_fmt["Sharpe"] = summary_fmt["Sharpe"].map(lambda x: f"{x:.3f}")
...
# ------------ Utilities ------------
def perf_from_equity_full(equity: pd.Series, rf: float):
    trading_days = 252
    years = max((equity.index[-1] - equity.index[0]).days / 365.25, 1e-9)
    ret_daily = equity.pct_change().dropna()
    ann_ret = ret_daily.mean() * trading_days
...
rebal_dates = set(prices.groupby(prices.index.to_period('M')).nth(0).index)
shares = None
last_rebal = None
...
# ------------ Performance stats ------------
stats_bh     = perf_from_equity_full(equity_bh,     risk_free_rate)
stats_daily  = perf_from_equity_full(equity_daily,  risk_free_rate)
stats_month  = perf_from_equity_full(equity_month,  risk_free_rate)
...
# format nicely
sf = summary.set_index("Strategy").copy()
for c in ["CAGR","Ann Ret","Ann Vol","Max DD"]:
    sf[c] = sf[c].map(lambda x: f"{x:.2%}")
sf["Sharpe"] = summary.set_index("Strategy")["Sharpe"].map(lambda x: f"{x:.3f}")
print(sf)
...
def load_prices(symbols, start_date, end_date):
    ...
    px = px.loc[:, cols_in].dropna(how='all').dropna(how='any')
    return px
